...
static struct hn_txdesc *hn_txd_get(struct uk_netdev_tx_queue *txq)
{
	struct hn_txdesc *txd;

	//if (rte_mempool_get(txq->txdesc_pool, (void **)&txd)) {
	txd = uk_allocpool_take(txq->txdesc_pool);
	if (txd == NULL) {
		++txq->stats.ring_full;
		// PMD_TX_LOG(DEBUG, "tx pool exhausted!");
		uk_pr_debug("tx pool exhausted!");
		return NULL;
	}
...
static void hn_txd_put(struct uk_netdev_tx_queue *txq, struct hn_txdesc *txd)
{
		 ...
		 struct hn_rx_bufinfo *rxb, void *buf, uint32_t len)
{
	...
	uk_pr_debug("[%s] begin hdr: %p\n", __func__, hdr);
	uk_pr_debug("[%s] begin hdr->type: %#x\n", __func__, hdr->type);
	switch (hdr->type) {
	case RNDIS_PACKET_MSG:
		uk_pr_debug("[%s] RNDIS_PACKET_MSG\n", __func__);
		// if (dev->data->dev_started)
			hn_rndis_rx_data(rxq, rxb, buf, len);
		break;

	case RNDIS_INDICATE_STATUS_MSG:
		uk_pr_debug("[%s] RNDIS_INDICATE_STATUS_MSG\n", __func__);
	...
	case RNDIS_INITIALIZE_CMPLT:
		uk_pr_debug("[%s] RNDIS_INITIALIZE_CMPLT\n", __func__);
		hn_rndis_receive_response(rxq->hv, buf, len);
		break;

	case RNDIS_QUERY_CMPLT:
		uk_pr_debug("[%s] RNDIS_QUERY_CMPLT\n", __func__);
		hn_rndis_receive_response(rxq->hv, buf, len);
		break;

	case RNDIS_SET_CMPLT:
		uk_pr_debug("[%s] RNDIS_SET_CMPLT\n", __func__);
		hn_rndis_receive_response(rxq->hv, buf, len);
		break;
			   ...
			   uint32_t tx_limit)
{
	...
	uk_pr_info("[%s] start queue_id: %d, tx_limit: %u\n", __func__, queue_id, tx_limit);
...
retry:
		...
		uk_pr_info("[%s] after vmbus_chan_recv_raw ret: %d\n", __func__, ret);
		if (ret == -EAGAIN)
			break;	/* ring is empty */
		...
		uk_pr_info("[%s] cph_type: %u\n", __func__, pkt->cph_type);
		switch (pkt->cph_type) {
		case VMBUS_CHANPKT_TYPE_COMP:
			uk_pr_debug("[%s] handle VMBUS_CHANPKT_TYPE_COMP\n", __func__);
			++tx_done;
			hn_nvs_handle_comp(dev, queue_id, pkt, data);
			break;

		case VMBUS_CHANPKT_TYPE_RXBUF:
			uk_pr_debug("[%s] handle VMBUS_CHANPKT_TYPE_RXBUF\n", __func__);
			hn_nvs_handle_rxbuf(dev, hv, rxq, pkt, data);
			break;

		case VMBUS_CHANPKT_TYPE_INBAND:
			uk_pr_debug("[%s] handle VMBUS_CHANPKT_TYPE_INBAND\n", __func__);
...
static int hn_flush_txagg(struct uk_netdev_tx_queue *txq, bool *need_sig)
{
	struct hn_txdesc *txd = txq->agg_txd;
...
static void hn_encap(struct rndis_packet_msg *pkt,
		     uint16_t queue_id,
		     const struct uk_netbuf *m)
{
	// unsigned int hlen = m->l2_len + m->l3_len;
	uint32_t *pi_data;
	uint32_t pkt_hlen;

	uk_pr_info("[%s] pkt: %p, queue_id: %d, m: %p\n", __func__, pkt, queue_id, m);
...
static unsigned int hn_get_slots(const struct uk_netbuf *m)
{
			       ...
			       const struct uk_netbuf *m)
{
	...
	while (m) {
		...
		while (len > 0) {
			...
			uk_pr_info("[%s] sg[%u].page: %u, sg[%u].ofs: %u, sg[%u].len: %u\n", __func__, segs, page, segs, offset, segs, bytes);
			sg[segs].page = page;
			sg[segs].ofs = offset;
			sg[segs].len = bytes;
			segs++;
...
// 		      bool *need_sig)
static int hn_xmit_sg(struct uk_netdev_tx_queue *txq,
		      const struct hn_txdesc *txd, const struct uk_netbuf *m,
		      bool *need_sig)
{
	...
	uk_pr_info("[%s] sg[0].page: %lu, sg[0].ofs: %u, sg[0].len: %u\n", __func__, sg[0].page, sg[0].ofs, sg[0].len);
...
// 	struct hn_tx_queue *txq = ptxq;
	...
	UK_ASSERT(txq != NULL);
	UK_ASSERT(m != NULL);
	UK_ASSERT(m->len < PAGE_SIZE);
	UK_ASSERT(!m->next); /* TODO: Support for netbuf chains missing */
	UK_ASSERT(((unsigned long) m->buf & ~PAGE_MASK) == 0);
...
//	 */
	...
	tx_thresh = MAX(txq->free_thresh, nb_pkts);
//	if (txq->queue_id == 0 ||
...
//		uint32_t pkt_size = m->pkt_len + HN_RNDIS_PKT_LEN;
		uint32_t pkt_size = m->len + HN_RNDIS_PKT_LEN;
		struct rndis_packet_msg *pkt;
		struct hn_txdesc *txd;
		...
		if (m->pkt_len <= hv->tx_copybreak &&
			...
			if (unlikely(ret != 0)) {
				if (ret == -EAGAIN) {
					...
					uk_pr_debug("sg channel full\n");
					++txq->stats.channel_full;
				} else {
					// PMD_DRV_LOG(NOTICE, "sg send failed: %d", ret);
					uk_pr_info("sg send failed: %d\n", ret);
					++txq->stats.errors;
				}
				hn_txd_put(txq, txd);
				goto fail;
...
// {
int hn_recv(struct uk_netdev *n __unused,
		struct uk_netdev_rx_queue *rxq,
		struct uk_netbuf **m)
{
	uk_pr_info("[%s] start\n");
